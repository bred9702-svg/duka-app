    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px 8px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
          <button
            onClick={() => addCash('in')}
            style={{ background: 'rgba(95,217,122,0.14)', color: '#5FD97A', border: '1px solid rgba(95,217,122,0.35)', borderRadius: 11, padding: '10px 8px', fontFamily: 'var(--font-display)', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
          >
            <Icon name="plus" size={15} /> Cash in
          </button>
          <button
            onClick={() => addCash('out')}
            style={{ background: 'rgba(255,107,91,0.14)', color: '#FF6B5B', border: '1px solid rgba(255,107,91,0.35)', borderRadius: 11, padding: '10px 8px', fontFamily: 'var(--font-display)', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
          >
        ...
        {unclassified.length === 0 ? (
          <Card style={{ textAlign: 'center', padding: 24, marginBottom: 12 }}>
            <Icon name="circleCheck" size={32} color="#5FD97A" style={{ display: 'block', margin: '0 auto 8px' }} />
            <p style={{ fontFamily: 'var(--font-display)', fontSize: 13, fontWeight: 600, color: 'var(--text-hi)', marginBottom: 4 }}>All clear</p>
            <p style={{ fontSize: 12, color: 'var(--text-low)' }}>No transactions to classify</p>
          </Card>
        ) : (
          <div>
